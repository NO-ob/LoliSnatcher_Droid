...
class ImageWriter{
  ...
  Future write(BooruItem item, bool jsonWrite, String booruName) async{
    ...
    try {
      ...
      if(SDKVer < 30){
        ...
        //} else {
         // Get.snackbar("File write error","Only jpg and png can be saved on android 11 currently",snackPosition: SnackPosition.BOTTOM,duration: Duration(seconds: 5),colorText: Colors.black, backgroundColor: Theme.of(context).primaryColor);
         // return 0;
        //}
  ...
  Stream<int> writeMultiple (List<BooruItem> snatched, bool jsonWrite, String booruName, int cooldown) async*{
    ...
    Get.snackbar("Snatching Complete","¡¡¡( •̀ ᴗ •́ )و!!!",snackPosition: SnackPosition.BOTTOM,duration: Duration(seconds: 2),colorText: Colors.black, backgroundColor: Theme.of(Get.context).primaryColor);
    if (existsList.length > 0){
      Get.snackbar("Some files were already snatched!", "File Count: ${existsList.length}", snackPosition: SnackPosition.BOTTOM, duration: Duration(seconds: 2), colorText: Colors.black, backgroundColor: Theme.of(Get.context).primaryColor);
    }
